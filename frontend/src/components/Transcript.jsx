...
export default function Transcript({ messages, mode }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 dark:bg-slate-900 dark:border-slate-800 shadow-sm flex flex-col h-[28rem] sm:h-[32rem] overflow-hidden">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-gray-50 dark:bg-slate-800/60"
      >
...
function TranscriptLine({ speaker, text }) {
  ...
  const agentBubble = `${bubbleBase} bg-white text-gray-800 rounded-tl-none border border-gray-200 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700`;
  ...
  return (
    ...
    >
      ...
      {!isAgent && (
        <div className="w-8 h-8 rounded-full bg-gray-200 text-gray-700 dark:bg-slate-700 dark:text-slate-100 flex items-center justify-center">
          <User size={16} />
        </div>
      )}
    </div>
